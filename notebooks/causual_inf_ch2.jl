...
# ╔═╡ b1231cdc-45ce-4f0f-b0ad-5c60e6ed4680
begin
	using Plots
	using PlutoUI
	TableOfContents() 
...
# ╔═╡ 538f329e-7b7f-48e6-80cd-92e786bcab43
begin
	using Random
	using StatsBase
	using Distributions
...
md"""
# Causual Inference in Julia: Chapter 2
**Author**: [Stefan Bringuier](https://stefanbringuier.info)

**Contact**: [stefanbringuier@gmail.com](mailto:stefanbringuier@gmail.com)
"""

# ╔═╡ e0028729-bd86-4404-9f63-ad1dd685eab3
md"""

**Source**: <https://github.com/PacktPublishing/Causal-Inference-and-Discovery-in-Python/blob/main/Chapter_02.ipynb>

**Book**: A. Molak, A. Jaokar, Causal inference and discovery in Python: unlock the secrets of modern causal machine learning with DoWhy, EconML, PyTorch and more, Packt Publishing Limited, Birmingham, 2023.


"""

# ╔═╡ 03d3c902-4878-4bed-bb9c-53515eb86330
md"""
## Association

Association is what we all know as correlation, where correlation is just the quantified magnitude and direction and association the qualitative notion. For example, given two variables, $x$ and $y$, how do they relate/change with respect to one another. In the language of Bayesian probablity we state:
...
which read as what is the probablity of $A$ given $B$ has occured (i.e., takes a value) or probablity of $B$ given $A$. What is important to note is that **we know nothing about what preceeds one another**. We do not know if $P(A|B)$ is invalid while $P(B|A)$ is valid because $A$ can never preceed $B$.
"""

# ╔═╡ 89723bb2-1998-4c6b-9b6e-7923c4219efa
md"""
## Structural Causual Model: Book Store
"""

# ╔═╡ 377e7af1-713f-48d8-be96-5908578a2ba0
md"""
!!! warning
    I'm using Turing.jl here because its nice to do probablistic programming, but its really not natural for this specific problem. However, I'll use it in later chapters and CausualInference.jl
"""

# ╔═╡ afd56077-cb34-461f-ba5f-e92dc17a31e1
@model function bookstore_scm()
	U₀ ~ Uniform()
	U₁ ~ Normal()
...
# ╔═╡ 24abd0f2-13b1-4a06-9ebf-3efa45d8406c
begin
	Random.seed!(45)
	model = bookstore_scm()
	nsamples = 100
	chain = sample(model, Prior(),nsamples)
	quants = generated_quantities(model,chain)
	results = hcat([Int(x[1]) for x in quants], [Int(x[2]) for x in quants])
end;

# ╔═╡ 6f661110-fb5f-4335-869c-3cd97a0a5715
md"""
Probablity of buying book A: $(sum(results[:,1])/nsamples)

Probability of buying book B: $(sum(results[:,2])/nsamples)

We can calculate the conditional probablities as:
"""

# ╔═╡ f54fd769-92b5-43a9-8827-85c1dfff7006
begin
	pAB = mean(results[results[:,2] .== 1, 1])
	pBA = mean(results[results[:,1] .== 1, 2])
end;

# ╔═╡ 4c1ec3e9-8bc6-4680-a9d3-62028c08ce9d
md"""
Probablity of buying book A given B was bought: $(pAB)

Probability of buying book B given A was bought: $(pBA)
"""

# ╔═╡ 2abbabbd-dd6c-466a-93dc-bbf5b6952d8c
md"""
## Intervention
"""
...
manifest_format = "2.0"
project_hash = "d63cc26e31bc19e5b2080c9dbb919b021ff92348"
...
git-tree-sha1 = "882510df0da64bae763a65240b07fb4b1c462317"
...
git-tree-sha1 = "cbf8fc8fbe40ede27407aaae37ad394ce4a02664"
