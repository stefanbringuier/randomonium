...
md"""
!!! warning
    Its possible reference ordering within the notebook get stale, so rerun the cell below or click "Refresh" button above.
"""

# ╔═╡ 43d6526a-4296-462c-ba99-e5130a42b724
begin
	refresh
	references = Dict(
...
# ╔═╡ 0484e6e2-9457-4a6e-98d8-56999d32ded5
begin
	function create_cite_function()
		ref_order = Dict{String, Int64}()
		order_list = DOI[]
		
		function cite(name::String)
			if !haskey(ref_order, name)
				ref_order[name] = length(ref_order) + 1
				push!(order_list, references[name])
			end
			
			return "[$(ref_order[name])]"
		end
		
		get_reference_order() = order_list
		
		return cite, get_reference_order
	end

	# Create an instance of the cite function and get_reference_order function
	cite, get_references = create_cite_function()
end;

# ╔═╡ b285e84c-6321-4d22-b4d4-28f830861ed3
@mdx """
Another reference  [$(cite("julia"))](#references). You have to modify the `references` dictionary in the [references](#references) section to add new entries. Then create a markdown link and use the `cite` function, for example:
...
@mdx """
This is a reference [$(cite("turing"))](#references)
"""

# ╔═╡ c68f5c8c-6e7a-4372-b6b7-2c4753020146
get_references()

# ╔═╡ 1a5d9ad0-009b-11ee-36d7-31a7b1bdd551
md"""
## Packages
$(let
...
begin
	initialize_eqref()
	ExtendedTableOfContents() # Uses PlutoExtras.jl
end

# ╔═╡ a5ff7542-e2a1-4ffe-9861-c285afd2de7f
md"""
### Plotting backend setup
"""
